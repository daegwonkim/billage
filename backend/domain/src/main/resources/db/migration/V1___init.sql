...
CREATE UNIQUE INDEX users_phone_no_active_uk
ON users (phone_no) WHERE is_withdrawn = false;
...
CREATE UNIQUE INDEX user_neighborhoods_user_id_uk
ON user_neighborhoods (user_id);
...
CREATE UNIQUE INDEX rental_item_like_records_user_item_uk
ON rental_item_like_records (user_id, rental_item_id);