...
enum class ErrorCode(
    ...
    // 인증 관련 에러
    INVALID_VERIFICATION_CODE("INVALID_VERIFICATION_CODE", "인증번호가 일치하지 않습니다", HttpStatus.BAD_REQUEST),
    ...
    NEIGHBORHOOD_NOT_FOUND("NEIGHBORHOOD_NOT_FOUND", "존재하지 않는 동네입니다", HttpStatus.NOT_FOUND),
    RENTAL_ITEM_NOT_FOUND("RENTAL_ITEM_NOT_FOUND", "존재하지 않는 상품입니다", HttpStatus.NOT_FOUND),

    UNSUPPORTED_REGION("UNSUPPORTED_REGION", "지원하지 않는 지역입니다", HttpStatus.BAD_REQUEST),

    // 서버 에러
    INTERNAL_SERVER_ERROR("INTERNAL_SERVER_ERROR", "서버 오류가 발생했습니다", HttpStatus.INTERNAL_SERVER_ERROR),
    EXTERNAL_API_ERROR("EXTERNAL_API_ERROR", "외부 API 요청에 실패했습니다", HttpStatus.BAD_GATEWAY)
}